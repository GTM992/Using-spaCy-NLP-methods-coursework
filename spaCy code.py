...
while True:
    ...
    if decision == "punctuation":
        ...
        if dec1 == "2":
            ...
            if dec2 == 'y':
                # Занесение знаменательных токенов (не являющихся знаками препинания и стоп-словами) в новый список.
                # Преобразование списка в строку. Извлечение всех знаков препинания, в том числе квадратных скобок (начало и конец списка).
                # Преобразование получившейся строки в Doc-объект.
                ...
                if dec3 == '1':
                    # Занесение токенов, не являющихся стоп-словами, в новый список.
                    test_doc = [token for token in test_doc if not token.is_stop]
                    # Извлечение знаков препинания от списка. 
                    test_doc = re.sub(r"[\](',')\[]", "", str(test_doc))                   
                    test_doc = nlp(re.sub(r"(, )", ' ', test_doc))

                elif dec3 == '2':
                    # Занесение токенов, не являющихся знаками препинания в новый список.
                    # Преобразование списка в строку. Извлечение всех знаков препинания, в том числе квадратных скобок (начало и конец списка).
                    # Преобразование получившейся строки в Doc-объект.
...
#Перебор токенов, вывод токенов, являющихся знаками препинания, вместе с указанием их индексов..
...
#Перебор токенов, вывод токенов, являющихся числами, вместе с указанием их индексов.
...
# Перебор токенов через однострочный итератор for, добавление знаменательных токенов 
# (не знаков препинания и не стоп-слов) в новый список (не doc-объект). Вывод длины исходного doc-объекта и списка.
...
# Перебор токенов через однострочный итератор for, добавление токенов, являющихся стоп-словами, в новый список. 
# Вывод каждого токена из нового списка.
...
# Перебор токенов через однострочный итератор for, добавление токенов, не являющихся стоп-словами и знаками препинания, в новый список.
# Вывод каждого токена из нового списка.
def filter(test_doc):
    test_doc_cleaned = [toke for toke in test_doc if not toke.is_stop and not toke.is_punct]
    for token in test_doc_cleaned:
        print(token.text)
if "filter" in decisions:
    filter(test_doc)


# Перебор токенов, вывод каждого из них рядом с его хэш-значением
...
# Перебор токенов, вывод каждого из них рядом с его начальной формой
...
# Перебор токенов, вывод тех, которые состоят из символов верхнего регистра, и добавление их в список.
# Список нужен для измерения его длины - для оповещения об отсутствии таких токенов в тексте.
...
# Перебор токенов, вывод каждого из них вместе с его частью речи по версии Morphologizer и Tagger
...
# Создание списка из множества предложений. Создание веб-сервера с визуализацией их структуры в виде дерева зависимостей.
...
# Перебор токенов в списке наименований, вывод каждого из них вместе с его типом.
...
# Инициализация Matcher на словаре данного Doc-объекта.
# Определение количества токенов в паттерне (в виде списка). Для каждого токена создаётся словарь, где ключ является критерием,
# а значение - требованием к данному критерию. Добавление паттерна в Matcher, нахождение совпадений в Doc-объекте. Вывод срезов из списка совпадений.
def match(test_doc):
...
# Вызов метода токена, где аргументом является другой токен. Токены получаются либо ручным вводом, либо индексами из Doc-объекта.
...
# Определение количества фраз в паттерне. 
